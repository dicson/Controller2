...
- **UI Development:** UI is managed via LVGL. The presence of `.eez-project` files suggests that UI layout is designed in EEZ Studio and then exported to the `src/ui/` directory. **Never modify `.c` files directly (e.g. in `src/ui/`), as they are generated by EEZ Studio.**
